...
namespace MyRevitExtractor
{
    ...
    public class glTFExportContext : IExportContext
    {
        ...
        public void OnMaterial(MaterialNode matNode)
        {
            ...
            Element m = (id != ElementId.InvalidElementId) ? _doc.GetElement(id) : null;

            // ★ [수정] 재질 요소(m)가 진짜로 존재하는지 확인
            if (m != null)
            {
                matName = m.Name;
                uniqueId = m.UniqueId;
            ...
            {
                // 재질 정보가 없으면 임의의 색상 이름 생성
                uniqueId = string.Format("r{0}g{1}b{2}", matNode.Color.Red, matNode.Color.Green, matNode.Color.Blue);
                matName = string.Format("Material_{0}", uniqueId);
            }
